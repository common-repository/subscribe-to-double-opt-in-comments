...
Plugin URI:   https://sjmp.de/internet/subscribe-to-comments-mit-double-opt-in-pruefung/
Description:  Allows readers to receive notifications via e-mail of new comments that are posted to a blog post. The first e-mail for a blog post is validated with a double-opt-in feature to ensure the user itself registered for notifications. Based on version 2 of "Subscribe to Comments" from Mark Jaquith (http://txfx.net/).
Version:      6.6.12
Author:       Tobias Herde
Author URI:   https://sjmp.de/
License:      GPL2
License URI:  https://www.gnu.org/licenses/gpl-2.0.html
Text Domain:  subscribe-to-double-opt-in-comments
...
function init_i10n() {
    ...
    $locale     = get_locale();
    if (!empty($locale)) {
        $domain_str = 'subscribe-to-double-opt-in-comments';
...
function show_manual_subscription_form() {
    ...
    sg_subscribe_start();
    $sg_subscribe->show_errors('solo_subscribe', '<div class="solo-subscribe-errors">', '</div>', __('<strong>Error: </strong>', 'subscribe-to-double-opt-in-comments'), '<br />');
...
?>
  ...
  <label for="solo-subscribe-email"><?php
        _e('E-Mail:', 'subscribe-to-double-opt-in-comments');
?>
  <input type="text" name="email" id="solo-subscribe-email" size="22" value="<?php
        echo $user_email;
?>" /></label>
  <input type="submit" name="submit" value="<?php
        _e('Subscribe', 'subscribe-to-double-opt-in-comments');
?>" />
  </p>
  </form>
...
class sg_subscribe_settings {
    function options_page_contents() {
        ...
        echo '<h2>' . __('Subscribe to Comments Options Page', 'subscribe-to-double-opt-in-comments') . '</h2> ';
		sprintf( __( '(by %s from <a href="%s" target="_blank">%s</a>, please <a rel="nofollow" href="%s" target="_blank">donate</a> if you like the plugin!)', 'subscribe-to-double-opt-in-comments'), 
            'Tobias H.',
			'https://sjmp.de/', 
			'sjmp.de', 
			$donationlink
        ...
        echo '<li><label for="name">' . __('"From" name for notifications:', 'subscribe-to-double-opt-in-comments') . ' <input type="text" size="40" id="name" name="sg_subscribe_settings[name]" value="' . sg_subscribe_settings::form_setting('name') . '" /></label></li>';
        echo '<li><label for="email">' . __('"From" e-mail addresss for notifications:', 'subscribe-to-double-opt-in-comments') . ' <input type="text" size="40" id="email" name="sg_subscribe_settings[email]" value="' . sg_subscribe_settings::form_setting('email') . '" /></label></li>';
        echo '<li><label for="clear_both"><input type="checkbox" id="clear_both" name="sg_subscribe_settings[clear_both]" value="true"' . sg_subscribe_settings::checkflag('clear_both') . ' /> ' . __('Do a CSS "clear" on the subscription checkbox/message (uncheck this if the checkbox/message appears in a strange location in your theme)', 'subscribe-to-double-opt-in-comments') . '</label></li>';
        echo '</ul>';
        
        echo '<fieldset><legend>' . __('Comment Form Text', 'subscribe-to-double-opt-in-comments') . '</legend>';
        
        echo '<p>' . __('Customize the messages shown to different people.  Use <code>[manager_link]</code> to insert the URI to the Subscription Manager.', 'subscribe-to-double-opt-in-comments') . '</p>';
        
        echo '<ul>';
        
        echo '<li><label for="not_subscribed_text">' . __('Not subscribed', 'subscribe-to-double-opt-in-comments') . '</label><br /><textarea style="width: 98%; font-size: 12px;" rows="2" cols="60" id="not_subscribed_text" name="sg_subscribe_settings[not_subscribed_text]">' . (sg_subscribe_settings::textarea_setting('not_subscribed_text')) . '</textarea></li>';
        
        echo '<li><label for="subscribed_text">' . __('Subscribed', 'subscribe-to-double-opt-in-comments') . '</label><br /><textarea style="width: 98%; font-size: 12px;" rows="2" cols="60" id="subscribed_text" name="sg_subscribe_settings[subscribed_text]">' . (sg_subscribe_settings::textarea_setting('subscribed_text')) . '</textarea></li>';
        
        echo '<li><label for="author_text">' . __('Entry Author', 'subscribe-to-double-opt-in-comments') . '</label><br /><textarea style="width: 98%; font-size: 12px;" rows="2" cols="60" id="author_text" name="sg_subscribe_settings[author_text]">' . (sg_subscribe_settings::textarea_setting('author_text')) . '</textarea></li>';
        
        echo '<li><label for="withoutCom_text">' . __('"Subscribe without Commenting" Text', 'subscribe-to-double-opt-in-comments') . '</label><br /><textarea style="width: 98%; font-size: 12px;" rows="2" cols="60" id="withoutCom_text" name="sg_subscribe_settings[withoutCom_text]">' . (sg_subscribe_settings::textarea_setting('withoutCom_text')) . '</textarea></li>';
        
        echo '<li><label for="confirmation_text">' . __('Confirmation Text (will be shown after user has clicked on the confirmation link)', 'subscribe-to-double-opt-in-comments') . '</label><br /><textarea style="width: 98%; font-size: 12px;" rows="2" cols="60" id="confirmation_text" name="sg_subscribe_settings[confirmation_text]">' . (sg_subscribe_settings::textarea_setting('confirmation_text')) . '</textarea></li>';
        
        echo '<li><label for="mail_text_head">' . __('Headline for Double-Opt-In', 'subscribe-to-double-opt-in-comments') . ' <input type="text" size="40" id="mail_text_head" name="sg_subscribe_settings[mail_text_head]" value="' . (sg_subscribe_settings::form_setting('mail_text_head')) . '" /></label></li>';
        
        echo '<li><label for="mail_text">' . __('Mail Text for Double-Opt-In ([verify_url] will be replaced with URL to subscribe finally, no HTML!)', 'subscribe-to-double-opt-in-comments') . '</label><br /><textarea style="width: 98%; font-size: 12px;" rows="8" cols="60" id="mail_text" name="sg_subscribe_settings[mail_text]">' . (sg_subscribe_settings::textarea_setting('mail_text')) . '</textarea></li>';
        
        echo '</ul></fieldset>';
        
        echo '<fieldset>';
        echo '<legend><input type="checkbox" id="use_custom_style" name="sg_subscribe_settings[use_custom_style]" value="true"' . sg_subscribe_settings::checkflag('use_custom_style') . ' /> <label for="use_custom_style">' . __('Use custom style for Subscription Manager', 'subscribe-to-double-opt-in-comments') . '</label></legend>';
        
        echo '<p>' . __('These settings only matter if you are using a custom style.  <code>[theme_path]</code> will be replaced with the path to your current theme.', 'subscribe-to-double-opt-in-comments') . '</p>';
        
        echo '<ul>';
        echo '<li><label for="sg_sub_header">' . __('Path to header:', 'subscribe-to-double-opt-in-comments') . ' <input type="text" size="40" id="sg_sub_header" name="sg_subscribe_settings[header]" value="' . sg_subscribe_settings::form_setting('header') . '" /></label></li>';
        echo '<li><label for="sg_sub_sidebar">' . __('Path to sidebar:', 'subscribe-to-double-opt-in-comments') . ' <input type="text" size="40" id="sg_sub_sidebar" name="sg_subscribe_settings[sidebar]" value="' . sg_subscribe_settings::form_setting('sidebar') . '" /></label></li>';
        echo '<li><label for="sg_sub_footer">' . __('Path to footer:', 'subscribe-to-double-opt-in-comments') . ' <input type="text" size="40" id="sg_sub_footer" name="sg_subscribe_settings[footer]" value="' . sg_subscribe_settings::form_setting('footer') . '" /></label></li>';
        
        
        echo '<li><label for="before_manager">' . __('HTML for before the subscription manager:', 'subscribe-to-double-opt-in-comments') . ' </label><br /><textarea style="width: 98%; font-size: 12px;" rows="2" cols="60" id="before_manager" name="sg_subscribe_settings[before_manager]">' . sg_subscribe_settings::textarea_setting('before_manager') . '</textarea></li>';
        echo '<li><label for="after_manager">' . __('HTML for after the subscription manager:', 'subscribe-to-double-opt-in-comments') . ' </label><br /><textarea style="width: 98%; font-size: 12px;" rows="2" cols="60" id="after_manager" name="sg_subscribe_settings[after_manager]">' . sg_subscribe_settings::textarea_setting('after_manager') . '</textarea></li>';
        echo '</ul>';
        echo '</fieldset>';
        
        echo '<fieldset>';
        echo '<legend>' . __('Hide copyright notice (please consider a donation via PayPal!)', 'subscribe-to-double-opt-in-comments') . '</legend>';
        
        echo '<p>' . __('You can hide the copyright notice published on the subscription manager page. This is optional. Please consider a donation via PayPal if you hide the copyright notice!', 'subscribe-to-double-opt-in-comments') . '</p>';
        echo '<p style="color:darkred;">' . __('Lots of work was and is put into this plugin, so be fair and donate if you hide the copyright notice!', 'subscribe-to-double-opt-in-comments');
        sprintf(__(' <a href="%s" rel="nofollow" target="_blank">Donate here!</a></p>', 'subscribe-to-double-opt-in-comments'), $donationlink);
        
        echo '<ul>';
        echo '<li><label for="hideCopyright"><input type="checkbox" id="hideCopyright" name="sg_subscribe_settings[hideCopyright]" value="true"' . sg_subscribe_settings::checkflag('hideCopyright') . ' /> ' . __('Hide the copyright notice from the subscription manager.', 'subscribe-to-double-opt-in-comments') . '</label></li>';
        echo '</ul>';
    ...
    function textarea_setting($optname) {
        $options = get_option('sg_subscribe_settings');
        return esc_html($options[$optname]);
    }
    
    function options_page() {
        /** Display "saved" notification on post **/
        if (isset($_POST['sg_subscribe_settings_submit']))
            echo '<div class="updated"><p><strong>' . __('Options saved.', 'subscribe-to-double-opt-in-comments') . '</strong></p></div>';
        ...
        _e('Update Options &raquo;', 'subscribe-to-double-opt-in-comments');
        echo '" /></p></div>';
...
class sg_subscribe {
    ...
    function __construct() {
        ...
        if (empty($this->settings['not_subscribed_text'])) {
            $this->not_subscribed_text = __('Notify me of followup comments via e-mail', 'subscribe-to-double-opt-in-comments');
            $this->subscribed_text     = __('You are subscribed to this entry. <a rel="nofollow" href="[manager_link]">Manage your subscriptions</a>.', 'subscribe-to-double-opt-in-comments');
            $this->author_text         = __('You are the author of this entry. <a rel="nofollow" href="[manager_link]">Manage subscriptions</a>.', 'subscribe-to-double-opt-in-comments');
            $this->mail_text           = __('Hello,\n\nplease click on the following link:\n[verify_url]\nto confirm your subscription.', 'subscribe-to-double-opt-in-comments');
            $this->mail_text_head      = __('Double Opt In E-Mail', 'subscribe-to-double-opt-in-comments');
            $this->clear_both          = $this->settings['clear_both'];
            $this->confirmation_text   = __('You will now get an e-mail, if a new comment is posted.', 'subscribe-to-double-opt-in-comments');
        } else {
    ...
    function subscriptions_from_email($email = '') {
        ...
        foreach ((array) $subscriptions as $subscription) {
			if (!is_array($this->email_subscriptions))
				$this->email_subscriptions = array();
    ...
    function solo_subscribe($email, $postid) {
        ...
        if (!is_email($email)) {
            ...
            if (is_email($user_email))
                ...
                $this->add_error(__('Please provide a valid e-mail address.', 'subscribe-to-double-opt-in-comments'), 'solo_subscribe');
        }
        
        if (($email == $this->site_email && is_email($this->site_email)) || ($email == get_option('admin_email') && is_email(get_option('admin_email'))))
            $this->add_error(__('This e-mail address may not be subscribed', 'subscribe-to-double-opt-in-comments'), 'solo_subscribe');
        ...
        if (is_array($this->subscriptions_from_email($email)))
            if (in_array($postid, (array) $this->subscriptions_from_email($email))) {
                ...
                $this->add_error(__('You appear to be already subscribed to this entry.', 'subscribe-to-double-opt-in-comments'), 'solo_subscribe');
            }
        ...
        if (!$post)
            $this->add_error(__('Comments are not allowed on this entry.', 'subscribe-to-double-opt-in-comments'), 'solo_subscribe');
        ...
        if (!is_array($this->errors['solo_subscribe'])) {
            ...
            $url_sub = get_option('home') . '/?wp-subscription-manager=1&cm=' . base64_encode($email) . '&pid=' . $postid . '&withoutCommentToken=' . $withoutToken . '&verify=' . $keyvalue;
    ...
    function add_subscriber($cid) {
        ...
        if (($_POST['subscribe'] == 'subscribe' && is_email($email)) || $previously_subscribed) {
            ...
            if ($test_user_mail->comment_author_email != $email && !$previously_subscribed && $test_user_mail->comment_approved != 'spam') {
                ...
                $url_sub  = get_option('home') . '/?wp-subscription-manager=1&verify=' . $keyvalue;
                $this->send_mail($email, $this->mail_text_head, str_replace("[verify_url]", $url_sub, $this->mail_text));
    ...
    function is_blocked($email = '') {
        ...
        $blocked = get_option('do_not_mail');
        if (!is_array(get_option('do_not_mail'))) {
    ...
    function remove_block($email = '') {
        ...
        if ($this->is_blocked($email)) {
            ...
            if (is_array(get_option('do_not_mail'))) {
                $blocked = str_replace($email, '', get_option('do_not_mail'));
            } else {
  ...
  <?php
    ...
    function send_notifications($cid) {
        ...
        if ($comment->comment_approved == '1' && $comment->comment_type == 'comment') {
            ...
            $message = sprintf(__("There is a new comment on the post \"%s\"", 'subscribe-to-double-opt-in-comments') . ". \n%s\n\n", $post->post_title, get_permalink($comment->comment_post_ID));
            $message .= sprintf(__("Author: %s\n", 'subscribe-to-double-opt-in-comments'), $comment->comment_author);
            $message .= __("Comment:\n", 'subscribe-to-double-opt-in-comments') . $comment->comment_content . "\n\n";
            ...
            $subject = sprintf(__('New Comment On: %s', 'subscribe-to-double-opt-in-comments'), $post->post_title);
    ...
    function change_email_request() {
        ...
        $message .= __('If you did not request this action, please disregard this message.', 'subscribe-to-double-opt-in-comments');
    ...
    function entry_link($postid, $uri = '') {
        ...
        if (empty($title))
            $title = __('click here', 'subscribe-to-double-opt-in-comments');
...
?>
    ...
    function db_upgrade_check() {
        ...
        if (is_multisite()) {
            ...
            add_option('sg_subscribe_settings', array(
                ...
                'before_manager' => '<div id="content" class="widecolumn subscription-manager">',
                'after_manager' => '</div>',
                'not_subscribed_text' => __('Notify me of followup comments via e-mail', 'subscribe-to-double-opt-in-comments'),
                'subscribed_text' => __('You are subscribed to this entry. <a rel="nofollow" href="[manager_link]">Manage your subscriptions</a>.', 'subscribe-to-double-opt-in-comments'),
                'author_text' => __('You are the author of this entry. <a rel="nofollow" href="[manager_link]">Manage subscriptions</a>.', 'subscribe-to-double-opt-in-comments'),
                'mail_text' => __("Hello,\n\nplease click on the following link:\n[verify_url]\nto confirm your subscription.", 'subscribe-to-double-opt-in-comments'),
                'mail_text_head' => __('Double Opt In', 'subscribe-to-double-opt-in-comments'),
                'version' => $this->version
            ));
        }
        ...
        if (!is_multisite()) {
            if (!isset($settings['confirmation_text'])) {
                $settings['confirmation_text'] = __('You will now get an e-mail, if a new comment is posted.', 'subscribe-to-double-opt-in-comments');
                $update                        = true;
            }
    ...
    function add_admin_menu() {
        add_management_page(__('Comment Subscription Manager', 'subscribe-to-double-opt-in-comments'), __('Subscriptions', 'subscribe-to-double-opt-in-comments'), 'manage_options', 'stc-management', 'sg_subscribe_admin');
        
        add_options_page(__('Subscribe to DOI Comments', 'subscribe-to-double-opt-in-comments'), __('Subscribe to DOI Comments', 'subscribe-to-double-opt-in-comments'), 'manage_options', 'stc-options', array(
...
add_action('comment_form_after_fields', 'show_subscription_checkbox');
add_action('comment_form_logged_in_after', 'show_subscription_checkbox');

// priority is very low (50) because we want to let anti-spam plugins have their way first.
add_action('comment_post', function($a){global $sg_subscribe; sg_subscribe_start(); return $sg_subscribe->send_notifications($a);}, 50);
add_action('comment_post', function($a){global $sg_subscribe; sg_subscribe_start(); return $sg_subscribe->add_subscriber($a);});

add_action('wp_set_comment_status', function($a){global $sg_subscribe; sg_subscribe_start(); return $sg_subscribe->send_notifications($a);});
add_action('admin_menu', function($a){global $sg_subscribe; sg_subscribe_start(); $sg_subscribe->add_admin_menu();});
add_action('admin_head', function($a){global $sg_subscribe; sg_subscribe_start(); $sg_subscribe->sg_wp_head();});

add_action('edit_comment', function($a){global $sg_subscribe; sg_subscribe_start(); return $sg_subscribe->on_edit($a);});
...
add_action('init', function($a){global $sg_subscribe; 
	if ( isset($_POST['solo-comment-subscribe']) && $_POST['solo-comment-subscribe'] == 'solo-comment-subscribe' && is_numeric($_POST['postid']) ) {
	  sg_subscribe_start();
...
function sg_subscribe_admin($standalone = false) {
    ...
    if ($standalone) {
        ...
        ob_start(function($a){return str_replace("<title>", "<title> " . __("Subscription Manager", "subscribe-to-double-opt-in-comments") . " &raquo; ", $a);});
    } else {
    ...
    if (!$sg_subscribe->validate_key() && strlen($_GET['verify']) != 15)
        die(__('You may not access this page without a valid key.', 'subscribe-to-double-opt-in-comments'));
    ...
    switch ($sg_subscribe->action):
        case "change_email":
            if ($sg_subscribe->change_email()) {
                $sg_subscribe->add_message(sprintf(__('All notifications that were formerly sent to <strong>%1$s</strong> will now be sent to <strong>%2$s</strong>!', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email, $sg_subscribe->new_email));
        ...
        case "remove_subscriptions":
            ...
            if ($postsremoved > 0)
                $sg_subscribe->add_message(sprintf(__('<strong>%1$s</strong> %2$s removed successfully.', 'subscribe-to-double-opt-in-comments'), $postsremoved, ($postsremoved != 1) ? __('subscriptions', 'subscribe-to-double-opt-in-comments') : __('subscription', 'subscribe-to-double-opt-in-comments')));
            break;
        
        case "remove_block":
            if ($sg_subscribe->remove_block($sg_subscribe->email))
                $sg_subscribe->add_message(sprintf(__('The block on <strong>%s</strong> has been successfully removed.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email));
            else
                $sg_subscribe->add_error(sprintf(__('<strong>%s</strong> isn\'t blocked!', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email), 'manager');
            break;
        
        case "email_change_request":
            if ($sg_subscribe->is_blocked($sg_subscribe->email))
                $sg_subscribe->add_error(sprintf(__('<strong>%s</strong> has been blocked from receiving notifications.  You will have to have the administrator remove the block before you will be able to change your notification address.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email));
            elseif ($sg_subscribe->change_email_request($sg_subscribe->email, $sg_subscribe->new_email))
                $sg_subscribe->add_message(sprintf(__('Your change of e-mail request was successfully received.  Please check your old account (<strong>%s</strong>) in order to confirm the change.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email));
            break;
        
        case "block_request":
            if ($sg_subscribe->block_email_request($sg_subscribe->email))
                $sg_subscribe->add_message(sprintf(__('Your request to block <strong>%s</strong> from receiving any further notifications has been received.  In order for you to complete the block, please check your e-mail and click on the link in the message that has been sent to you.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email));
            break;
        
        case "solo_subscribe":
            $sg_subscribe->add_message(sprintf(__('<strong>%1$s</strong> has been successfully subscribed to %2$s', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email, $sg_subscribe->entry_link($_GET['subscribeid'])));
            break;
        
        case "block":
            if ($sg_subscribe->add_block($sg_subscribe->email))
                $sg_subscribe->add_message(sprintf(__('<strong>%1$s</strong> has been added to the "do not mail" list. You will no longer receive any notifications from this site. If this was done in error, please contact the <a rel="nofollow" href="mailto:%2$s">site administrator</a> to remove this block.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email, $sg_subscribe->site_email));
            else
                $sg_subscribe->add_error(sprintf(__('<strong>%s</strong> has already been blocked!', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email), 'manager');
            $sg_subscribe->key = $sg_subscribe->generate_key($sg_subscribe->email);
            $sg_subscribe->validate_key();
            break;
...
?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
  ...
  <head>
  <title><?php
            printf(__('%s Comment Subscription Manager', 'subscribe-to-double-opt-in-comments'), bloginfo('name'));
?></title>
...
?>
  <?php
        $sg_subscribe->add_message(sprintf(__('Return to the page you were viewing: %s', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->entry_link(url_to_postid($sg_subscribe->ref), $sg_subscribe->ref)));
?>
  <?php
...
?>
  ...
  <?php
    if ($_GET['userSubscribeWithoutComment'] == "true") {
        echo __('You will get an e-mail to verify your subscription. Please follow the steps described within this e-mail.', 'subscribe-to-double-opt-in-comments');
    }
    ...
    if (strlen($verify_code) == 15 && empty($withoutCommentToken)) {
        ...
        if (!empty($check_exists->comment_author_email)) {
            ...
            echo '<div class="verify_failed">' . __('Error! Comment does not exist anymore!', 'subscribe-to-double-opt-in-comments') . '</div>';
        }
        ...
        if ($withoutCommentTokenCookie == $withoutCommentToken) {
            ...
            echo '<div class="verify_failed">' . __('Error! Wrong security token submitted, please use the same browser and allow cookies!', 'subscribe-to-double-opt-in-comments') . '</div>';
        }
    }
?>
...
?>
    ...
    <fieldset class="options">
      <legend><?php
            _e('Remove Block', 'subscribe-to-double-opt-in-comments');
?></legend>

      <p>
      <?php
            printf(__('Click the button below to remove the block on <strong>%s</strong>.  This should only be done if the user has specifically requested it.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email);
?>
      </p>
...
?>
      ...
      <p class="submit">
      <input type="submit" name="submit" value="<?php
            _e('Remove Block &raquo;', 'subscribe-to-double-opt-in-comments');
?>" />
      </p>
      </form>
    </fieldset>
...
?>
    ...
    <fieldset class="options">
      <legend><?php
            _e('Blocked', 'subscribe-to-double-opt-in-comments');
?></legend>

      <p>
      <?php
            printf(__('You have indicated that you do not wish to receive any notifications at <strong>%1$s</strong> from this site. If this is incorrect, or if you wish to have the block removed, please contact the <a rel="nofollow" href="mailto:%2$s">site administrator</a>.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email, $sg_subscribe->site_email);
?>
      </p>
    </fieldset>
...
<?php
        if (isset($sg_subscribe->email) && !is_array($postlist) && $sg_subscribe->email != $sg_subscribe->site_email && $sg_subscribe->email != get_bloginfo('admin_email')) {
            if (is_email($sg_subscribe->email))
                $sg_subscribe->add_error(sprintf(__('<strong>%s</strong> is not subscribed to any posts on this site.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email));
            else
                $sg_subscribe->add_error(sprintf(__('<strong>%s</strong> is not a valid e-mail address.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email));
        }
?>
...
?>
      ...
      <legend><?php
            _e('Find Subscriptions', 'subscribe-to-double-opt-in-comments');
?></legend>

      <p>
      <?php
            _e('Enter an e-mail address to view its subscriptions or undo a block.', 'subscribe-to-double-opt-in-comments');
?>
      </p>
...
?>" />
      ...
      <input type="submit" value="<?php
            _e('Search &raquo;', 'subscribe-to-double-opt-in-comments');
?>" />
      </p>
      </form>
    </fieldset>
...
?>
        <legend><?php
                    _e('Top Subscriber List', 'subscribe-to-double-opt-in-comments');
?></legend>
      <?php
                else:
?>
        <legend><?php
                    _e('Subscriber List', 'subscribe-to-double-opt-in-comments');
?></legend>
      <?php
                endif;
?>
...
?>"><?php
                    _e('Show all subscribers', 'subscribe-to-double-opt-in-comments');
?></a></p>
<?php
...
?>"><?php
                    _e('Show list of subscribers in <code>CC:</code>-field format (for bulk e-mailing)', 'subscribe-to-double-opt-in-comments');
?></a></p>
<?php
                else:
?>
...
?>"><?php
                    _e('&laquo; Back to regular view', 'subscribe-to-double-opt-in-comments');
?></a></p>
                ...
                if ($all_subscriptions) {
                    if (!$_REQUEST['showccfield']) {
                        ...
                        foreach ((array) $all_subscriptions as $email => $ccount) {
                            ...
                            echo "<li>($ccount) <a rel='nofollow' href='" . esc_attr($sg_subscribe->form_action . "&email=$enc_email") . "'>" . esc_html($email) . "</a></li>\n";
                        }
                        echo "</ul>\n";
                    }
?>
        <legend><?php
                    _e('Top Subscribed Posts', 'subscribe-to-double-opt-in-comments');
?></legend>
        <?php
                    ...
                    foreach (array(
                        'top_subscribed_posts1',
                        'top_subscribed_posts2'
                    ) as $each) {
...
//-->
    ...
    <fieldset class="options">
      <legend><?php
            _e('Subscriptions', 'subscribe-to-double-opt-in-comments');
?></legend>

        <p>
        <?php
            printf(__('<strong>%s</strong> is subscribed to the posts listed below. To unsubscribe to one or more posts, click the checkbox next to the title, then click "Remove Selected Subscription(s)" at the bottom of the list.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email);
?>
        </p>
...
?>
        ...
        <a rel="nofollow" href="javascript:;" onclick="checkAll(document.getElementById('removeSubscription')); return false; "><?php
            _e('Invert Checkbox Selection', 'subscribe-to-double-opt-in-comments');
?></a>
        </p>

        <p class="submit">
        <input type="submit" name="submit" value="<?php
            _e('Remove Selected Subscription(s) &raquo;', 'subscribe-to-double-opt-in-comments');
?>" />
        </p>
        </form>
    </fieldset>
  ...
  <div class="wrap">
  <h2><?php
            _e('Advanced Options', 'subscribe-to-double-opt-in-comments');
?></h2>

    <fieldset class="options">
      <legend><?php
            _e('Block All Notifications', 'subscribe-to-double-opt-in-comments');
?></legend>
...
?>
        ...
        <p>
        <?php
            printf(__('If you would like <strong>%s</strong> to be blocked from receiving any notifications from this site, click the button below.  This should be reserved for cases where someone is signing you up for notifications without your consent.', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email);
?>
        </p>

        <p class="submit">
        <input type="submit" name="submit" value="<?php
            _e('Block Notifications &raquo;', 'subscribe-to-double-opt-in-comments');
?>" />
        </p>
        </form>
    </fieldset>
    ...
    <fieldset class="options">
      <legend><?php
            _e('Change E-mail Address', 'subscribe-to-double-opt-in-comments');
?></legend>
...
?>
        ...
        <p>
        <?php
            printf(__('If you would like to change the e-mail address for your subscriptions, enter the new address below.  You will be required to verify this request by clicking a special link sent to your current address (<strong>%s</strong>).', 'subscribe-to-double-opt-in-comments'), $sg_subscribe->email);
?>
        </p>

        <p>
        <?php
            _e('New E-mail Address:', 'subscribe-to-double-opt-in-comments');
?> 
        ...
        <p class="submit">
        <input type="submit" name="submit" value="<?php
            _e('Change E-mail Address &raquo;', 'subscribe-to-double-opt-in-comments');
?>" />
        </p>
        </form>
    </fieldset>
